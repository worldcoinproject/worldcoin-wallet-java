...
import com.google.worldcoin.core.CoreTestUtils.BlockPair;
...
import static com.google.worldcoin.core.Utils.toNanoCoins;
import static org.junit.Assert.*;
import static org.junit.Assert.assertEquals;

/**
 * This is a selection of the worldcoinj wallet tests - mainly included
 * to exercise the WorldcoinWallet code every time the wallet tests are run.
 * 
 * The full wallet tests are in the worldcoinj codebase.
 * 
 * @author jim
 *
 */
public class WalletTest {
    ...
    private Transaction sendMoneyToWallet(Wallet wallet, Transaction tx, AbstractBlockChain.NewBlockType type) throws IOException,
        ...
        if (type == null) {
            ...
            BlockPair bp = com.google.worldcoin.core.CoreTestUtils.createFakeBlock(params, blockStore, tx);
    ...
    private Transaction sendMoneyToWallet(Wallet wallet, BigInteger value, Address toAddress, AbstractBlockChain.NewBlockType type)
        ...
        return sendMoneyToWallet(wallet, com.google.worldcoin.core.CoreTestUtils.createFakeTx(params, value, toAddress), type);
    }

    private Transaction sendMoneyToWallet(BigInteger value, AbstractBlockChain.NewBlockType type) throws IOException,
            VerificationException {
        return sendMoneyToWallet(this.wallet, com.google.worldcoin.core.CoreTestUtils.createFakeTx(params, value, myAddress), type);
    }
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, boolean testEncryption) throws Exception {
        ...
        // will attach a small fee. Because the Worldcoin protocol makes it
  ...
  @Test
   public void largeBalance() throws Exception {
    ...
    public void balances() throws Exception {
        ...
        // Reserialize.
        Transaction send2 = new Transaction(params, send1.worldcoinSerialize());
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = new Transaction(params, t1.worldcoinSerialize());
        com.google.worldcoin.core.CoreTestUtils.BlockPair fakeBlock = CoreTestUtils.createFakeBlock(params, blockStore, t1Copy);
    ...
    public void pending2() throws Exception {
        ...
        // Our balance is now 0.50 WDC less a fee.
    ...
    private void encryptionDecryptionBasicCommon(Wallet wallet) {
        ...
        assertTrue("WorldcoinWallet is not an encrypted wallet", wallet.getEncryptionType() == EncryptionType.ENCRYPTED_SCRYPT_AES);
        ...
        // WorldcoinWallet should now be unencrypted.
        assertTrue("WorldcoinWallet is not an unencrypted wallet", wallet.getKeyCrypter() == null);
        ...
        // WorldcoinWallet should now be of type WalletType.ENCRYPTED_SCRYPT_AES.
        assertTrue("WorldcoinWallet is not an encrypted wallet", wallet.getEncryptionType() == EncryptionType.ENCRYPTED_SCRYPT_AES);
    }
    ...
    public void encryptionDecryptionBadPassword() throws Exception {
        ...
        assertTrue("WorldcoinWallet is not an encrypted wallet", encryptedWallet.getEncryptionType() == EncryptionType.ENCRYPTED_SCRYPT_AES);
    ...
    public void encryptionDecryptionCheckExceptions() throws Exception {
        ...
        assertTrue("WorldcoinWallet is not an unencrypted wallet.2", encryptedWallet.getKeyCrypter() == null);

        // Encrypt wallet.
        encryptedWallet.encrypt(keyCrypter, aesKey);

        assertTrue("WorldcoinWallet is not an encrypted wallet.2", encryptedWallet.getEncryptionType() == EncryptionType.ENCRYPTED_SCRYPT_AES);
        ...
        assertTrue("WorldcoinWallet is not an encrypted wallet.3", encryptedWallet.getEncryptionType() == EncryptionType.ENCRYPTED_SCRYPT_AES);
    }
    ...
    public void encryptionDecryptionHomogenousKeys() throws Exception {
        ...
        // This is not allowed as the ScryptParameters is stored at the WorldcoinWallet level.
        byte[] salt = new byte[KeyCrypterScrypt.SALT_LENGTH];
        secureRandom.nextBytes(salt);
        Protos.ScryptParameters.Builder scryptParametersBuilder = Protos.ScryptParameters.newBuilder().setSalt(ByteString.copyFrom(salt));
        ScryptParameters scryptParameters = scryptParametersBuilder.build();
