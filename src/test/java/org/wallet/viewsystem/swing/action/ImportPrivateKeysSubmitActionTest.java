...
import org.wallet.file.PrivateKeysHandlerTest;
...
public class ImportPrivateKeysSubmitActionTest extends TestCase {   
    ...
    public void testImportUnencryptedPrivateKeysWithUnencryptedWallet() throws Exception { 
        ...
        assertTrue("WorldcoinWallet password is enabled when it should not be", !importPanel.isWalletPasswordFieldEnabled());
    ...
    public void testImportUnencryptedPrivateKeysWithEncryptedWallet() throws Exception { 
        ...
        assertTrue("WorldcoinWallet is not encrypted when it should be",  controller.getModel().getActiveWallet().getEncryptionType() == EncryptionType.ENCRYPTED_SCRYPT_AES);
        ...
        assertNotNull("importAction was not created successfully", importAction);
        assertEquals("Wrong number of keys at wallet creation", 1, controller.getModel().getActiveWallet().getKeychain().size());
        assertTrue("WorldcoinWallet password is not enabled when it should be", importPanel.isWalletPasswordFieldEnabled());
    ...
    public void testNoWalletSelected() throws Exception {
        ...
        assertEquals("Wrong message after receive worldcoin confirm with no active wallet", ResetTransactionsSubmitActionTest.EXPECTED_NO_WALLET_IS_SELECTED, ((Message)messages[messages.length - 1]).getText());
    }
    
    private void checkEveryExpectedKeyIsPresent(WorldcoinController controller) {
        // Check every key on the expected list is now on the wallet.
