...
import org.wallet.viewsystem.swing.action.WorldcoinWalletAction;
 ...
 * <p/>
 * In WorldcoinWallet the StatusBar is responsible for :
 * <p/>
...
public class StatusBar extends JPanel implements MessageListener {
  ...
  final private WorldcoinWalletButton statusLabel;
   ...
   */
  public StatusBar(WorldcoinController worldcoinController, WorldcoinWalletFrame mainFrame) {
    ...
    final WorldcoinController finalController = this.worldcoinController;
    ...
    onlineLabel.addMouseListener(new MouseAdapter() {
      ...
      public void mouseEntered(MouseEvent arg0) {
        int blockHeight = -1;
        if (finalController.getWorldcoinWalletService() != null) {
          if (finalController.getWorldcoinWalletService().getChain() != null) {
            if (finalController.getWorldcoinWalletService().getChain().getChainHead() != null) {
              blockHeight = finalController.getWorldcoinWalletService().getChain().getChainHead().getHeight();
              Block blockHeader = finalController.getWorldcoinWalletService().getChain().getChainHead().getHeader();
              Date blockTime = blockHeader.getTime();
              int numberOfPeers = 0;
              if (finalController.getWorldcoinWalletService().getPeerGroup() != null && finalController.getWorldcoinWalletService().getPeerGroup().getConnectedPeers() != null) {
                numberOfPeers = finalController.getWorldcoinWalletService().getPeerGroup().getConnectedPeers().size();
              }
              onlineLabel.setToolTipText(HelpContentsPanel.createMultilineTooltipText(new String[]{
                      finalController.getLocaliser().getString("worldcoinWalletFrame.numberOfBlocks",
                              new Object[]{blockHeight}),
                      finalController.getLocaliser().getString("worldcoinWalletFrame.blockDate",
                              new Object[]{dateFormatter.format(blockTime)}),
                      finalController.getLocaliser().getString("worldcoinWalletFrame.connectedTo",
    ...
    statusLabel = new WorldcoinWalletButton("");
    ...
    WorldcoinWalletAction showMessagesAction = new WorldcoinWalletAction(controller, null, null,
    ...
    String tooltipText = HelpContentsPanel.createMultilineTooltipText(new String[]{
            controller.getLocaliser().getString("worldcoinWalletFrame.statusBar.tooltip1"), "\n",
            controller.getLocaliser().getString("worldcoinWalletFrame.statusBar.tooltip2")});
    ...
    int onlineWidth = Math.max(Math.max(
            getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont()).stringWidth(
                    controller.getLocaliser().getString("worldcoinWalletFrame.onlineText")),
            getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont()).stringWidth(
                    controller.getLocaliser().getString("worldcoinWalletFrame.offlineText"))),
            getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont()).stringWidth(
                    controller.getLocaliser().getString("worldcoinWalletFrame.errorText"))
    )
  ...
  public void updateOnlineStatusTextOnSwingThread(final StatusEnum finalStatusEnum) {
    ...
    if (finalStatusEnum == StatusEnum.ONLINE) {
      ...
      if (mainFrame != null) {
        BlinkLabel estimatedBalanceWDCLabel = mainFrame.getEstimatedBalanceWDCLabel();
        if (estimatedBalanceWDCLabel != null) {
          estimatedBalanceWDCLabel.setBlinkEnabled(true);
        }
    ...
    if (finalStatusEnum == StatusEnum.ERROR) {
      ...
      String toolTip = HelpContentsPanel.createMultilineTooltipText(new String[]{
              controller.getLocaliser().getString("worldcoinWalletFrame.statusBar.error1"),
              controller.getLocaliser().getString("worldcoinWalletFrame.statusBar.error2")});
