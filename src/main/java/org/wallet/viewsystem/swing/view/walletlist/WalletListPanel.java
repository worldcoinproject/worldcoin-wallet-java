...
import org.wallet.viewsystem.swing.WorldcoinWalletTabbedPane;
...
public class WalletListPanel extends JPanel implements Viewable, WalletBusyListener, ComponentListener, CurrencyConverterListener {
    ...
    private WorldcoinWalletTabbedPane tabbedPane;
     ...
     */
    public WalletListPanel(WorldcoinController worldcoinController, WorldcoinWalletFrame mainFrame) {
    ...
    private void displayView(DisplayHint displayHint, boolean blinkEnabled) {
        if (walletPanels != null) {
            synchronized(walletPanels) {
                ...
                for (SingleWalletPanel loopSingleWalletPanel : walletPanels) {
                    ...
                    boolean modelBlinkEnabled = this.worldcoinController.getModel().isBlinkEnabled();
        ...
        String activeWalletFilename = this.worldcoinController.getModel().getActiveWalletFilename();
        WalletData activePerModelData = this.worldcoinController.getModel().getPerWalletModelDataByWalletFilename(activeWalletFilename);
    ...
    public void initUI() {
        ...
        tabbedPane = new WorldcoinWalletTabbedPane(controller);
            ...
            synchronized (walletPanels) {
                for (WalletData loopPerWalletModelData : perWalletModelDataList) {
                    if (loopPerWalletModelData.getWallet() != null) {
                        ...
                        SingleWalletPanel loopPanel = new SingleWalletPanel(loopPerWalletModelData, this.worldcoinController, mainFrame, this);
    ...
    private JPanel createButtonPanel() {
        ...
        CreateWalletSubmitAction createNewWalletAction = new CreateWalletSubmitAction(this.worldcoinController, ImageLoader.createImageIcon(ImageLoader.CREATE_NEW_ICON_FILE), mainFrame);
        WorldcoinWalletButton createNewWalletButton = new WorldcoinWalletButton(createNewWalletAction, controller);
        ...
        if (walletPanels != null) {
            synchronized (walletPanels) {
                ...
                for (SingleWalletPanel loopSingleWalletPanel : walletPanels) {
                    if (loopSingleWalletPanel.getPerWalletModelData().getWalletFilename() != null) {
                        if (loopSingleWalletPanel.getPerWalletModelData().getWalletFilename()
                                .equals(this.worldcoinController.getModel().getActiveWalletFilename())) {
                ...
                if (nextSelectedWalletIndex > -1) {
                    this.worldcoinController.getModel().setActiveWalletByFilename(walletPanels.get(nextSelectedWalletIndex).getPerWalletModelData().getWalletFilename());
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (selectedWalletPanel != null) {
                ...
                if (!selectedWalletPanel.getPerWalletModelData().getWalletFilename()
                        .equals(worldcoinController.getModel().getActiveWalletFilename())) {
                    worldcoinController.getModel()
    ...
    public void componentResized(ComponentEvent arg0) {
        ...
        if (scrollPane.getVerticalScrollBar().isVisible()) {
            preferredWalletWidth -= WorldcoinWalletFrame.SCROLL_BAR_DELTA;
        }         
