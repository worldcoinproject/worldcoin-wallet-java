...
public class ShowTransactionsPanel extends JPanel implements Viewable, CurrencyConverterListener {
    ...
    private WorldcoinWalletButton showTransactionsButton;
    
    private Action exportTransactionsSubmitAction;
    private WorldcoinWalletButton exportTransactionsButton;
    ...
    public ShowTransactionsPanel(WorldcoinController worldcoinController, WorldcoinWalletFrame mainFrame) {
    ...
    private JPanel createTransactionsPanel() {
        ...
        walletTableModel = new WalletTableModel(worldcoinController);
        ...
        tableColumn = table.getColumnModel().getColumn(3); // Amount (WDC).
        int amountWDCWidth = Math.max(fontMetrics.stringWidth(controller.getLocaliser().getString("sendWorldcoinPanel.amountLabel") + " (WDC)"),
                fontMetrics.stringWidth("00000.000000000"));
        tableColumn.setPreferredWidth(amountWDCWidth);
        tableColumn.setMinWidth(amountWDCWidth);

        if (CurrencyConverter.INSTANCE.isShowingFiat()) {
            tableColumn = table.getColumnModel().getColumn(4); // Amount (fiat).
            int amountFiatWidth = Math.max(fontMetrics.stringWidth(controller.getLocaliser().getString("sendWorldcoinPanel.amountLabel") + " (USD)"),
    ...
    private JPanel createButtonPanel() {
        ...
        showTransactionDetailsAction = new ShowTransactionDetailsAction(worldcoinController, mainFrame, this);
        showTransactionsButton = new WorldcoinWalletButton(showTransactionDetailsAction, controller);
        ...
        exportTransactionsSubmitAction = new ExportTransactionsSubmitAction(worldcoinController, mainFrame);
        exportTransactionsButton = new WorldcoinWalletButton(exportTransactionsSubmitAction, controller);
        showTransactionsButton.setEnabled(false);
        constraints.fill = GridBagConstraints.NONE;
        constraints.gridx = 2;
        constraints.gridy = 0;
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
            ...
            case BUILDING: {
                if (worldcoinController.getWorldcoinWalletService().getChain() == null) {
                    ...
                    int numberOfBlocksEmbedded = worldcoinController.getWorldcoinWalletService().getChain().getBestChainHeight() - confidence.getAppearedAtChainHeight() + 1;
                    if (transaction != null && transaction.isCoinBase()) {
                    ...
                    if (numberOfBlocksEmbedded >= 6) {
                        primaryLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("worldcoinWalletFrame.status.isConfirmed")));
                    } else {
                        if (transaction != null && transaction.isCoinBase()) {
                            primaryLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("worldcoinWalletFrame.status.beingConfirmedAndCoinbase")));
                        } else {
                            primaryLabel.setToolTipText(HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("worldcoinWalletFrame.status.beingConfirmed")));
                        }
                    }
                }
                break;
            ...
            case DEAD: {
                ...
                primaryLabel.setText(controller.getLocaliser().getString("worldcoinWalletFrame.status.dead"));
                break;
            }
            default: {
                primaryLabel.setIcon(null);
        ...
        private String getUnconfirmedConfidenceToolTip(Transaction transaction) {
            ...
            if (transaction != null) {
                if (transaction.getLockTime() > 0) {
                    ...
                    transactionTrustfulness = WorldcoinWallet.getController().getLocaliser().getString("worldcoinWalletFrame.status.notConfirmedAndNotStandard") + ".";
                } else {
                    // Normal transaction.
                    if (transaction.isCoinBase()) {
                        transactionTrustfulness = WorldcoinWallet.getController().getLocaliser().getString("worldcoinWalletFrame.status.notConfirmedAndCoinbase") + ".";
                    } else {
                        transactionTrustfulness = WorldcoinWallet.getController().getLocaliser().getString("worldcoinWalletFrame.status.notConfirmed") + ".";
                    }
                }
            }
    ...
    class TrailingJustifiedNumericRenderer extends DefaultTableCellRenderer {
        ...
        WorldcoinWalletLabel label;

        public TrailingJustifiedNumericRenderer() {
            label = new WorldcoinWalletLabel("");
        }
    ...
    class DecimalAlignRenderer implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row,
            ...
            if (value == null) {
                pane.setText("\t" + controller.getLocaliser().worldcoinValueToString(BigInteger.ZERO, false, false));
            } else {
