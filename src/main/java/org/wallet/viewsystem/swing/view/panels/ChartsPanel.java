...
public class ChartsPanel extends JPanel implements Viewable, ComponentListener {
  ...
  private static final double NUMBER_OF_SATOSHI_IN_ONE_WDC = 100000000;
   ...
   */
    public ChartsPanel(WorldcoinController worldcoinController, WorldcoinWalletFrame mainFrame) {
  ...
  private JPanel createChartPanel() {
    try {
        ...
        if (chartDataCollection == null || chartDataCollection.size() == 0) {
          ...
          for (ChartData chartData : chartDataCollection) {
            if (chartData != null && chartData.getDate() != null && chartData.getValue() != null) {
              ...
              yData.add(chartData.getValue().doubleValue() / NUMBER_OF_SATOSHI_IN_ONE_WDC);
            }
          }
        }
      }
      ...
      String currencyUnitSuffix = " (" + controller.getLocaliser().getString("sendWorldcoinPanel.amountUnitLabel") + ")";
      String balanceLabel = controller.getLocaliser().getString("worldcoinWalletFrame.balanceLabel") + currencyUnitSuffix;
  ...
  private Collection<ChartData> getChartData() {
    ...
    if (controller.getModel() == null || this.worldcoinController.getModel().getActiveWallet() == null) {
      return new ArrayList<ChartData>();
    }

    ArrayList<Transaction> allTransactions = new ArrayList<Transaction>(this.worldcoinController.getModel().getActiveWallet().getTransactions(false));
    ...
    try {
      ...
      if (allTransactions == null || allTransactions.size() == 0) {
        ...
        for (Transaction loop : allTransactions) {
          balance = balance.add(loop.getValue(this.worldcoinController.getModel().getActiveWallet()));
    ...
    } catch (com.google.worldcoin.core.ScriptException e1) {
      e1.printStackTrace();
    }
