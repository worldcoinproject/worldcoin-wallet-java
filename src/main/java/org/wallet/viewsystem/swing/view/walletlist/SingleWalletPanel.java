...
import org.wallet.network.WorldcoinWalletDownloadListener;
...
import org.wallet.viewsystem.swing.view.components.WorldcoinWalletTextField;
...
public class SingleWalletPanel extends JPanel implements ActionListener, FocusListener, WalletBusyListener {
  ...
  private WorldcoinWalletTextField walletDescriptionTextField;
  private Border walletDescriptionTextFieldBorder;

  private BlinkLabel amountLabelWDC;
  ...
  public SingleWalletPanel(WalletData perWalletModelData, final WorldcoinController worldcoinController, WorldcoinWalletFrame mainFrame, final WalletListPanel walletListPanel) {
    ...
    perWalletModelData.setSingleWalletDownloadListener(new SingleWalletPanelDownloadListener(this.worldcoinController, this));
    ...
    walletDescriptionTextField = new WorldcoinWalletTextField(perWalletModelData.getWalletDescription(), WIDTH_OF_TEXT_FIELD,
            controller);
    ...
    myRoundedPanel.add(WorldcoinWalletTitledPanel.createStent(2, 2), constraints);
    ...
    constraints.gridheight = 1;
    constraints.anchor = GridBagConstraints.LINE_END;
    JPanel iconRowStent = WorldcoinWalletTitledPanel.createStent((int) (walletDescriptionTextFieldBorder.getBorderInsets(this).left * 0.5));
    myRoundedPanel.add(iconRowStent, constraints);

    // WorldcoinWallet type icon.
    ...
    twistyStent = WorldcoinWalletTitledPanel.createStent(1, 1);
    ...
    amountLabelWDC = new BlinkLabel(controller, false);
    amountLabelWDC.setBackground(ColorAndFontConstants.BACKGROUND_COLOR);
    amountLabelWDC.setBorder(BorderFactory.createEmptyBorder(0, 3, 0, 3));
    amountLabelWDC.setBlinkEnabled(true);
    amountLabelWDC.setHorizontalAlignment(JLabel.TRAILING);
    ...
    constraints.gridheight = 1;
    constraints.anchor = GridBagConstraints.LINE_END;
    myRoundedPanel.add(amountLabelWDC, constraints);
  ...
  public static int calculateNormalWidth(JComponent component) {
    ...
    return (int) (fontMetrics.stringWidth(WorldcoinWalletFrame.EXAMPLE_MEDIUM_FIELD_TEXT) + WIDTH_DELTA);
  }
  ...
  public void addMouseListener(MouseListener mouseListener) {
    ...
    amountLabelWDC.addMouseListener(mouseListener);
  ...
  public void setBusyIconStatus(boolean isBusy) {
    ...
    if (this.worldcoinController.getModel().getActivePerWalletModelData().isBusy()) {
      ...
      if (perWalletModelData != null) {
        String toolTipText = HelpContentsPanel.createTooltipText(controller.getLocaliser().getString("worldcoinWalletSubmitAction.walletIsBusy",
  ...
  private void saveChanges() {
    if (!perWalletModelData.isFilesHaveBeenChangedByAnotherProcess()) {
      ...
      String titleText = controller.getLocaliser().getString("worldcoinWalletFrame.title");
      if (this.worldcoinController.getModel().getActiveWallet() != null) {
        titleText = titleText + WorldcoinWalletFrame.SEPARATOR
                + this.worldcoinController.getModel().getActivePerWalletModelData().getWalletDescription() + WorldcoinWalletFrame.SEPARATOR
                + this.worldcoinController.getModel().getActivePerWalletModelData().getWalletFilename();
      }
  ...
  public void updateFromModel(boolean blinkEnabled, boolean useBusyStatus) {
    ...
    String balanceTextToShowWDC = controller.getLocaliser().worldcoinValueToString(estimatedBalance, true, false);
    ...
    if (useBusyStatus && perWalletModelData.isBusy()) {
      ...
      if (amountLabelWDC != null && amountLabelWDC.getText() != null && !"".equals(amountLabelWDC.getText())
              && !balanceTextToShowWDC.equals(amountLabelWDC.getText()) && blinkEnabled) {
        amountLabelWDC.blink(balanceTextToShowWDC);
        amountLabelFiat.blink(balanceTextToShowFiat);
      } else {
        amountLabelWDC.setText(balanceTextToShowWDC);
    ...
    if (perWalletModelData.isFilesHaveBeenChangedByAnotherProcess()) {
      ...
      walletDescriptionTextField.setEditable(false);
      amountLabelWDC.setText("");
      amountLabelWDC.setEnabled(false);
  ...
  public void setSyncMessage(String message, double syncPercent) {
    ...
    if (syncPercent > WorldcoinWalletDownloadListener.DONE_FOR_DOUBLES) {
      ...
      amountLabelWDC.setBlinkEnabled(false);
      amountLabelFiat.setBlinkEnabled(false);

      amountLabelWDC.setText(message);
      amountLabelFiat.setText(percentText);

      amountLabelWDC.setBlinkEnabled(true);
      amountLabelFiat.setBlinkEnabled(true);

      if (perWalletModelData.getWalletFilename().equals(this.worldcoinController.getModel().getActiveWalletFilename())) {
