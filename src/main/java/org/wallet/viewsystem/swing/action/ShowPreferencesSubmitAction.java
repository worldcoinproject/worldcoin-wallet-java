...
public class ShowPreferencesSubmitAction extends AbstractAction {
     ...
     */
    public ShowPreferencesSubmitAction(WorldcoinController worldcoinController, ExchangeController exchangeController, PreferencesDataProvider dataProvider, Icon icon,
            WorldcoinWalletFrame mainFrame) {
        super(worldcoinController.getLocaliser().getString("showPreferencesSubmitAction.text"), icon);
...
//                controller.getModel().setUserPreference(WorldcoinModel.PREVIOUS_SEND_FEE, previousSendFee);
//
//                // Check fee is set.
//                if (newSendFee == null || "".equals(newSendFee)) {
//                    // Fee must be set validation error.
//                    controller.getModel().setUserPreference(WorldcoinModel.SEND_FEE, previousSendFee);
...
//                        if (feeAsBigInteger.compareTo(WorldcoinModel.SEND_MINIMUM_FEE) < 0) {
//                            feeValidationError = true;
//                            updateStatusText = controller.getLocaliser().getString(
//                                    "showPreferencesPanel.feeCannotBeSmallerThanMinimumFee");
//                        } else {
//                            if (feeAsBigInteger.compareTo(WorldcoinModel.SEND_MAXIMUM_FEE) >= 0) {
//                                feeValidationError = true;
//                                updateStatusText = controller.getLocaliser().getString(
//                                        "showPreferencesPanel.feeCannotBeGreaterThanMaximumFee");
//                            } else {
//                                // Fee is ok.
//                                controller.getModel().setUserPreference(WorldcoinModel.SEND_FEE, newSendFee);
//                            }
//                        }
...
//                }
            ...
            if (openUriDialog != null) {
                controller.getModel().setUserPreference(WorldcoinModel.OPEN_URI_SHOW_DIALOG, openUriDialog);
            }

            // Open URI - use URI.
            String openUriUseUri = dataProvider.getOpenUriUseUri();
            if (openUriUseUri != null) {
                controller.getModel().setUserPreference(WorldcoinModel.OPEN_URI_USE_URI, openUriUseUri);
            }
            ...
            boolean showWorldcoinConvertedToFiat = dataProvider.getNewShowWorldcoinConvertedToFiat();
            ...
            } else if (dataProvider.getPreviousShowWorldcoinConvertedToFiat() != showWorldcoinConvertedToFiat) {
                wantToFireDataStructureChanged = true;
                if (showWorldcoinConvertedToFiat) {
                    restartTickerTimer = true;
                }
            ...
            controller.getModel().setUserPreference(ExchangeModel.SHOW_WORLDCOIN_CONVERTED_TO_FIAT,
              Boolean.valueOf(showWorldcoinConvertedToFiat).toString());
            ...
            if (newCurrency1 != null && !newCurrency1.equals(previousCurrency1)) {
                ...
                if (ExchangeData.WORLDCOIN_CHARTS_EXCHANGE_NAME.equals(newExchange1)) {
            ...
            if (wantToFireDataStructureChanged || lookAndFeelHasChanged) {
                ...
                // Switch off blinks.
                this.worldcoinController.getModel().setBlinkEnabled(false);
                ...
                try {
                    ...
                    this.worldcoinController.getModel().setBlinkEnabled(true);
                }            
            }
