...
/**
 * WorldcoinWallet info is the companion info to the worldcoinj WorldcoinWallet that wallet uses
...
public class WalletInfoData {
    ...
    private static final String INFO_MAGIC_TEXT = "worldcoinWallet.info";
    ...
    private WorldcoinWalletVersion walletVersion;
    private com.google.worldcoin.core.Wallet wallet;
     ...
     */
    public WalletInfoData(String walletFilename, com.google.worldcoin.core.Wallet wallet, WorldcoinWalletVersion walletVersion) {
        ...
        if (wallet != null) {
            for (ECKey key : wallet.getKeys()) {
                if (receivingAddress.getAddress().equals(
                        key.toAddress(WorldcoinWallet.getWorldcoinController().getModel().getNetworkParameters()).toString())) {
     ...
     */
    public void checkAllReceivingAddressesAppearInWallet(com.google.worldcoin.core.Wallet wallet) {
     ...
     */
    public void writeToFile(String walletInfoFilename, WorldcoinWalletVersion walletVersion) throws WalletSaveException {
        ...
        try {
            ...
            walletPreferencesClone.remove(WorldcoinModel.WALLET_FILE_SIZE);
            walletPreferencesClone.remove(WorldcoinModel.WALLET_FILE_LAST_MODIFIED);
            walletPreferencesClone.remove(WorldcoinModel.WALLET_INFO_FILE_SIZE);
            walletPreferencesClone.remove(WorldcoinModel.WALLET_INFO_FILE_LAST_MODIFIED);

            walletPreferencesClone.remove(WorldcoinModel.VALIDATION_ADDRESS_IS_INVALID);
            walletPreferencesClone.remove(WorldcoinModel.VALIDATION_ADDRESS_VALUE);
            walletPreferencesClone.remove(WorldcoinModel.VALIDATION_AMOUNT_IS_INVALID);
            walletPreferencesClone.remove(WorldcoinModel.VALIDATION_AMOUNT_IS_MISSING);
            walletPreferencesClone.remove(WorldcoinModel.VALIDATION_AMOUNT_IS_NEGATIVE_OR_ZERO);
            walletPreferencesClone.remove(WorldcoinModel.VALIDATION_AMOUNT_VALUE);
            walletPreferencesClone.remove(WorldcoinModel.VALIDATION_NOT_ENOUGH_FUNDS);

            walletPreferencesClone.remove(WorldcoinModel.SEND_PERFORM_PASTE_NOW);
        ...
        try {
            ...
            if (walletVersionTokenNumber == 2) {
                ...
                if (!WALLET_VERSION_MARKER.equals(walletVersionMarker)
                        || !(WorldcoinWalletVersion.SERIALIZED.getWalletVersionString().equals(walletVersionString)
                                || WorldcoinWalletVersion.PROTOBUF.getWalletVersionString().equals(walletVersionString) || WorldcoinWalletVersion.PROTOBUF_ENCRYPTED
                    ...
                    if (!walletVersion.getWalletVersionString().equals(walletVersionString)) {
                        ...
                        if (WorldcoinWalletVersion.SERIALIZED.getWalletVersionString().equals(walletVersionString)) {
                            walletVersion = WorldcoinWalletVersion.SERIALIZED;
                        } else if (WorldcoinWalletVersion.PROTOBUF.getWalletVersionString().equals(walletVersionString)) {
                            walletVersion = WorldcoinWalletVersion.PROTOBUF;
                        } else if (WorldcoinWalletVersion.PROTOBUF_ENCRYPTED.getWalletVersionString().equals(walletVersionString)) {
                            walletVersion = WorldcoinWalletVersion.PROTOBUF_ENCRYPTED;
                        }
                    }
                }
            } else {
    ...
    public static String createWalletInfoFilename(String walletFilename) {
        ...
        if (walletFilename.endsWith(WorldcoinModel.WALLET_FILE_EXTENSION)) {
            walletInfoFilename = walletInfoFilename.substring(0,
                    walletFilename.length() - WorldcoinModel.WALLET_FILE_EXTENSION.length() - 1);
    ...
    public static String decodeURLString(String stringToDecode) {
        try {
            // Earlier worldcoin-wallets did not encode the info file - check if
            // encoded.
    ...
    public WorldcoinWalletVersion getWalletVersion() {
        return walletVersion;
    }

    public void setWalletVersion(WorldcoinWalletVersion walletVersion) {
        this.walletVersion = walletVersion;
    }
}
