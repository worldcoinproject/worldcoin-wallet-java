...
/**
 * Model containing the WorldcoinWallet data.
 * 
 ...
 * 
 */
public class WorldcoinModel extends AbstractModel<CoreModel> {

      private static final Logger log = LoggerFactory.getLogger(WorldcoinModel.class);

    // Constants used in the wallet.properties.

    // WorldcoinWallet start up.
    ...
    // Send worldcoin and send worldcoin confirm.
    ...
    // Maximum fee.
    public static final BigInteger SEND_MAXIMUM_FEE = new BigInteger("100000000"); // 1 WDC.

    // Receive worldcoin.
    public static final String IS_RECEIVE_WORLDCOIN = "isReceiveWorldcoin";
    ...
    public WorldcoinModel(CoreModel coreModel) {
        super(coreModel);
    ...
    public void setActiveWalletPreference(String key, String value) {
        if (WorldcoinModel.SEND_AMOUNT.equals(key)) {
            if (value.contains(",")) {
                boolean bad = true;
                bad = !bad;
            }
     ...
     */
    public WalletData addWallet(final WorldcoinController worldcoinController, Wallet wallet, String walletFilename) {
        if (walletFilename == null) {
            return null;
        }
        ...
        if (wallet != null) {
            wallet.addEventListener(worldcoinController);
        }

        createWalletTableData(worldcoinController, walletFilename);
     ...
     */
    public ArrayList<WalletTableData> createActiveWalletData(final WorldcoinController worldcoinController) {
        return createWalletTableData(worldcoinController, this.getActivePerWalletModelData());
    }
     ...
     * to show to the user in tabular form.
     */
    public ArrayList<WalletTableData> createWalletTableData(final WorldcoinController worldcoinController, String walletFilename) {
        ...
        return createWalletTableData(worldcoinController, perWalletModelData);
    }

    public ArrayList<WalletTableData> createWalletTableData(final WorldcoinController worldcoinController, WalletData perWalletModelData) {
        ArrayList<WalletTableData> walletData = new ArrayList<WalletTableData>();
        ...
        if (transactions != null) {
            for (Transaction loopTransaction : transactions) {
                ...
                if (transactionInputs != null) {
                    ...
                    if (firstInput != null) {
                        walletDataRow.setDescription(createDescription(worldcoinController, perWalletModelData.getWallet(), transactionInputs,
                                transactionOutputs, walletDataRow.getCredit(), walletDataRow.getDebit()));
                    }
                }
                walletDataRow.setDate(createDate(worldcoinController, loopTransaction));
     ...
     */
    private Date createDate(final WorldcoinController worldcoinController, Transaction transaction) {
                ...
                if (iterator.hasNext()) {
                    ...
                    try {
                        if (worldcoinController != null && worldcoinController.getWorldcoinWalletService() != null
                                && worldcoinController.getWorldcoinWalletService().getBlockStore() != null) {
                            appearsInStoredBlock = worldcoinController.getWorldcoinWalletService().getBlockStore().get(appearsInHash);
    ...
    public NetworkParameters getNetworkParameters() {
        ...
        String testOrProduction = super.getUserPreference(WorldcoinModel.TEST_OR_PRODUCTION_NETWORK);
        if (testOrProduction == null) {
            testOrProduction = WorldcoinModel.PRODUCTION_NETWORK_VALUE;
            super.setUserPreference(WorldcoinModel.TEST_OR_PRODUCTION_NETWORK, testOrProduction);
        }
        if (WorldcoinModel.TEST_NETWORK_VALUE.equalsIgnoreCase(testOrProduction)) {
            return NetworkParameters.testNet2();
        } else if (WorldcoinModel.TESTNET3_VALUE.equalsIgnoreCase(testOrProduction)) {
