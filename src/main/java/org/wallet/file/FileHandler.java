...
import org.wallet.store.WorldcoinWalletProtobufSerializer;
...
/**
 * Class consolidating the File IO in WorldcoinWallet for wallets and wallet infos.
 * 
 * @author jim
 * 
 */
public class FileHandler {
    ...
    public static final String USER_PROPERTIES_FILE_NAME = "worldcoin-wallet.properties";
    ...
    private WorldcoinWalletProtobufSerializer walletProtobufSerializer;
    ...
    public FileHandler(WorldcoinController worldcoinController) {
        this.worldcoinController = worldcoinController;
        this.controller = this.worldcoinController;

        walletProtobufSerializer = new WorldcoinWalletProtobufSerializer();
    }
    ...
    public WalletData loadFromFile(File walletFile) throws WalletLoadException, WalletVersionException {
        ...
        try {
            ...
            if (isWalletSerialised(walletFile)) {
                ...
                walletInfo = new WalletInfoData(walletFilenameToUseInModel, null, WorldcoinWalletVersion.PROTOBUF_ENCRYPTED);
            }
             ...
             if (!walletWasLoadedSuccessfully) {
                 ...
                 while (!walletWasLoadedSuccessfully && iterator.hasNext()) {
                    ...
                    try {
                        ...
                        MessageManager.INSTANCE.addMessage(new Message(worldcoinController.getLocaliser().getString("fileHandler.walletCannotLoadUsingBackup",
            ...
            if (walletWasLoadedSuccessfully) {
                ...
                wallet.setNetworkParameters(worldcoinController.getModel().getNetworkParameters());

                perWalletModelData = worldcoinController.getModel().addWallet(this.worldcoinController, wallet,
                ...
                if (useBackupWallets) {
                    ...
                    walletInfo.put(WorldcoinModel.WALLET_BACKUP_FILE, "");
                ...
                // Report failure to user.
                String messageText = worldcoinController.getLocaliser().getString("fileHandler.unableToLoadWalletOrBackups", new String[] {walletFilenameToUseInModel});
                if (!errorMessages.isEmpty()) {
                    ...
                    messageText = messageText + "\n" + worldcoinController.getLocaliser().getString("deleteWalletConfirmDialog.walletDeleteError2", new String[]{errorMessagesAsString.toString()});
                }
    ...
    public void saveWalletAndWalletInfoSimple(WalletData perWalletModelData, String walletFilename, String walletInfoFilename) {
        ...
        try {
            if (perWalletModelData.getWallet() != null) {
                // WorldcoinWallet description is currently stored in the wallet info
                ...
                if (WorldcoinWalletVersion.SERIALIZED == walletInfo.getWalletVersion()) {
                    ...
                    if (walletIsActuallyEncrypted) {
                        walletInfo.setWalletVersion(WorldcoinWalletVersion.PROTOBUF_ENCRYPTED);
                    }

                    if (WorldcoinWalletVersion.PROTOBUF == walletInfo.getWalletVersion()) {
                        // Save as a WorldcoinWallet message.
                        perWalletModelData.getWallet().saveToFile(walletFile);
                    } else if (WorldcoinWalletVersion.PROTOBUF_ENCRYPTED == walletInfo.getWalletVersion()) {
                        fileOutputStream = new FileOutputStream(walletFile);

                        // Save as a WorldcoinWallet message with a mandatory extension
                        ...
                        throw new WalletVersionException("Cannot save wallet '" + perWalletModelData.getWalletFilename()
                                ...
                                + "' but this version of WorldcoinWallet does not understand that format.");
                    }
                }
    ...
    private void saveWalletAndWalletInfo(WalletData perWalletModelData, String walletFilename, String walletInfoFilename) {
        ...
        try {
            if (perWalletModelData.getWallet() != null) {
                ...
                String oldBackupFilename = perWalletModelData.getWalletInfo().getProperty(WorldcoinModel.WALLET_BACKUP_FILE);
                ...
                if (WorldcoinWalletVersion.PROTOBUF == walletInfo.getWalletVersion()
                        || WorldcoinWalletVersion.PROTOBUF_ENCRYPTED == walletInfo.getWalletVersion()) {
                ...
                if (WorldcoinWalletVersion.PROTOBUF == walletInfo.getWalletVersion()
                        || WorldcoinWalletVersion.PROTOBUF_ENCRYPTED == walletInfo.getWalletVersion()) {
                    perWalletModelData.getWalletInfo().put(WorldcoinModel.WALLET_BACKUP_FILE, newBackupFilename);
    ...
    public File backupPrivateKeys(CharSequence passwordToUse) throws IOException, KeyCrypterException {
        ...
        if (passwordToUse != null && passwordToUse.length() > 0) {
            if (controller.getModel() != null
                    && this.worldcoinController.getModel().getActiveWalletInfo() != null
                    && this.worldcoinController.getModel().getActiveWalletInfo().getWalletVersion() == WorldcoinWalletVersion.PROTOBUF_ENCRYPTED) {
                // Save a backup copy of the private keys, encrypted with the passwordToUse.
                PrivateKeysHandler privateKeysHandler = new PrivateKeysHandler(this.worldcoinController.getModel()
                        .getNetworkParameters());
                String privateKeysBackupFilename = BackupManager.INSTANCE.createBackupFilename(new File(this.worldcoinController.getModel()
                        .getActiveWalletFilename()), BackupManager.PRIVATE_KEY_BACKUP_DIRECTORY_NAME, false, false, WorldcoinModel.PRIVATE_KEY_FILE_EXTENSION);
                privateKeysBackupFile = new File(privateKeysBackupFilename);
                BlockChain blockChain = null;
                if (this.worldcoinController.getWorldcoinWalletService() != null) {
                    blockChain = this.worldcoinController.getWorldcoinWalletService().getChain();
                }

                privateKeysHandler.exportPrivateKeys(privateKeysBackupFile, this.worldcoinController.getModel().getActiveWallet(),
                        blockChain, true, passwordToUse, passwordToUse);
            } else {
                log.debug("WorldcoinWallet '" + this.worldcoinController.getModel().getActiveWalletFilename()
    ...
    private String copyExistingWalletToBackupAndDeleteOriginal(File walletFile) throws IOException {
        String newWalletBackupFilename = BackupManager.INSTANCE.createBackupFilename(walletFile, BackupManager.ROLLING_WALLET_BACKUP_DIRECTORY_NAME, false, false, WorldcoinModel.WALLET_FILE_EXTENSION);
    ...
    public boolean haveFilesChanged(WalletData perWalletModelData) {
        ...
        if (walletInfo != null) {
            synchronized (walletInfo) {
                ...
                if (!walletFileSize.equals(walletInfo.getProperty(WorldcoinModel.WALLET_FILE_SIZE))) {
                    haveFilesChanged = true;
                }

                if (!walletFileLastModified.equals(walletInfo.getProperty(WorldcoinModel.WALLET_FILE_LAST_MODIFIED))) {
                    haveFilesChanged = true;
                }

                if (!walletInfoFileSize.equals(walletInfo.getProperty(WorldcoinModel.WALLET_INFO_FILE_SIZE))) {
                    haveFilesChanged = true;
                }
                ...
                if (haveFilesChanged) {
                    ...
                    log.debug(WorldcoinModel.WALLET_FILE_SIZE + " " + walletFileSize + " ," + WorldcoinModel.WALLET_FILE_LAST_MODIFIED
                            + " " + walletFileLastModified + " ," + WorldcoinModel.WALLET_INFO_FILE_SIZE + " " + walletInfoFileSize
                            + " ," + WorldcoinModel.WALLET_INFO_FILE_LAST_MODIFIED + " " + walletInfoFileLastModified);
                }
            }
        }
    ...
    private void rememberFileSizesAndLastModified(File walletFile, WalletInfoData walletInfo) {
        ...
        walletInfo.put(WorldcoinModel.WALLET_FILE_SIZE, "" + walletFileSize);
        walletInfo.put(WorldcoinModel.WALLET_FILE_LAST_MODIFIED, "" + walletFileLastModified);
        walletInfo.put(WorldcoinModel.WALLET_INFO_FILE_SIZE, "" + walletInfoFileSize);
        walletInfo.put(WorldcoinModel.WALLET_INFO_FILE_LAST_MODIFIED, "" + walletInfoFileLastModified);

        log.debug("rememberFileSizesAndLastModified: WorldcoinWallet filename " + walletFilename + " , " + WorldcoinModel.WALLET_FILE_SIZE
                + " " + walletFileSize + " ," + WorldcoinModel.WALLET_FILE_LAST_MODIFIED + " " + walletFileLastModified + " ,"
                + WorldcoinModel.WALLET_INFO_FILE_SIZE + " " + walletInfoFileSize + " ,"
                + WorldcoinModel.WALLET_INFO_FILE_LAST_MODIFIED + " " + walletInfoFileLastModified);
    }

    public static void writeUserPreferences(WorldcoinController worldcoinController) {
        final Controller controller = worldcoinController;
        // Save all the wallets' filenames in the user preferences.
        if (worldcoinController.getModel().getPerWalletModelDataList() != null) {
            ...
            for (WalletData perWalletModelData : perWalletModelDataList) {
                ...
                if (perWalletModelData.getWalletInfo().getWalletVersion() == WorldcoinWalletVersion.PROTOBUF_ENCRYPTED) {
                    if (!protobuf3List.contains(perWalletModelData.getWalletFilename())) {
                        ...
                        || perWalletModelData.getWalletInfo().getWalletVersion() == WorldcoinWalletVersion.SERIALIZED
                        || perWalletModelData.getWalletInfo().getWalletVersion() == WorldcoinWalletVersion.PROTOBUF) {
            ...
            for (String walletFilename : orderList) {
                controller.getModel().setUserPreference(WorldcoinModel.WALLET_ORDER_PREFIX + orderCount, walletFilename);
                orderCount++;
            }
            controller.getModel().setUserPreference(WorldcoinModel.WALLET_ORDER_TOTAL, "" + orderList.size());

            int earlyCount = 1;
            for (String walletFilename : earlyList) {
                controller.getModel().setUserPreference(WorldcoinModel.EARLY_WALLET_FILENAME_PREFIX + earlyCount, walletFilename);
                earlyCount++;
            }
            controller.getModel().setUserPreference(WorldcoinModel.NUMBER_OF_EARLY_WALLETS, "" + earlyList.size());

            int protobuf3Count = 1;
            for (String walletFilename : protobuf3List) {
                controller.getModel().setUserPreference(WorldcoinModel.PROTOBUF3_WALLET_FILENAME_PREFIX + protobuf3Count,
                        walletFilename);
                protobuf3Count++;
            }
            controller.getModel().setUserPreference(WorldcoinModel.NUMBER_OF_PROTOBUF3_WALLETS, "" + protobuf3List.size());
            controller.getModel().setUserPreference(WorldcoinModel.ACTIVE_WALLET_FILENAME,
                    worldcoinController.getModel().getActiveWalletFilename());
        }
     ...
     * the user's application data directory when WorldcoinWallet is first used.
     * 
    ...
    public void copyCheckpointsFromInstallationDirectory(String destinationCheckpointsFilename) throws IOException {
        ...
        if (!destinationCheckpoints.exists()) {
            ...
            String filePrefix = WorldcoinWalletService.getFilePrefix();
            String checkpointsFilename = filePrefix + WorldcoinWalletService.CHECKPOINTS_SUFFIX;
    ...
    public void copyBlockChainFromInstallationDirectory(String destinationBlockChainFilename, boolean alwaysOverWrite)
        ...
        if (!destinationBlockchain.exists() || alwaysOverWrite) {
            ...
            String filePrefix = WorldcoinWalletService.getFilePrefix();
            String blockchainFilename = filePrefix + WorldcoinWalletService.BLOCKCHAIN_SUFFIX;
