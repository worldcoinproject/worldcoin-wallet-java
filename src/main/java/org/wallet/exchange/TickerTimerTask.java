...
public class TickerTimerTask extends TimerTask {
     ...
     */
    public TickerTimerTask(ExchangeController exchangeController, WorldcoinWalletFrame mainFrame, boolean isFirstExchange) {
    ...
    public void run() {
        ...
        try {
            ...
            if (marketDataService != null) {
                    ...
                    if (!Boolean.FALSE.toString().equals(controller.getModel().getUserPreference(ExchangeModel.TICKER_SHOW))
                        ...
                        for (CurrencyPair loopSymbolPair : exchangeSymbols) {
                            if (ExchangeData.OPEN_EXCHANGE_RATES_EXCHANGE_NAME.equals(shortExchangeName)) {
                                ...
                                if ("WDC".equals(loopSymbolPair.baseCurrency) && loopSymbolPair.counterCurrency.equals(currency)) {
                                    getItFromTheServer = true;
                                    currencyPairToUse = loopSymbolPair;
                                    break;
                                }
                                if ("WDC".equals(loopSymbolPair.counterCurrency) && loopSymbolPair.baseCurrency.equals(currency)) {
                                    getItFromTheServer = true;
                                    invertedRates = true;
                                    currencyPairToUse = loopSymbolPair;
                                    break;
                        ...
                        if (getItFromTheServer) {
                            ...
                            if (ExchangeData.OPEN_EXCHANGE_RATES_EXCHANGE_NAME.equals(shortExchangeName)) {
                                ...
                                Ticker wdcUsdTicker = null;
                                log.debug("Getting wdcUsdTicker for WDC/USD");
                                wdcUsdTicker = marketDataService.getTicker(CurrenciesLocal.WDC, Currencies.USD);
                                System.out.println("wdcUsdTicker = " + wdcUsdTicker);

                                BigMoney usdWdcRateMoney = wdcUsdTicker.getLast();
                                BigDecimal usdWdcRate = null;
                                if (usdWdcRateMoney != null) {
                                    usdWdcRate = usdWdcRateMoney.getAmount();
                                    if (loopTicker.getLast() != null) {
                                        last = loopTicker.getLast().dividedBy(usdWdcRate, RoundingMode.HALF_EVEN);
                                    }
                                    if (loopTicker.getBid() != null) {
                                        bid = loopTicker.getBid().dividedBy(usdWdcRate, RoundingMode.HALF_EVEN);
                                    }
                                    if (loopTicker.getAsk() != null) {
                                        ask = loopTicker.getAsk().dividedBy(usdWdcRate, RoundingMode.HALF_EVEN);
                                    }
                                }
                            } else {
                                ...
                                if (invertedRates) {
                                    if (reverseRates) {
                                        // USD/ WDC, reciprocal rate
                                        currency = currencyPairToUse.baseCurrency;
                                    } else {
                                        // WDC/ USD, reciprocal rate
                            ...
                            if (isFirstExchange) {
                                ...
                                if (ExchangeData.WORLDCOIN_CHARTS_EXCHANGE_NAME.equals(shortExchangeName)) {
                ...
                // Fire exchange rate data changed - used by rest of WorldcoinWallet.
            ...
            if (exchangeSymbols != null) {
                ...
                for (int i = 0; i < exchangeSymbols.size(); i++) {
                    ...
                    if (ExchangeData.OPEN_EXCHANGE_RATES_EXCHANGE_NAME.equals(newExchangeName)) {
                        if ("USD".equalsIgnoreCase(baseCurrency) && !"WDC".equalsIgnoreCase(counterCurrency)) {
                        ...
                        if ("USD".equalsIgnoreCase(counterCurrency) && !"WDC".equalsIgnoreCase(baseCurrency)) {
                        ...
                        if ("WDC".equalsIgnoreCase(baseCurrency)) {
                            availableCurrencies.add(counterCurrency);
                        }
                        if ("WDC".equalsIgnoreCase(counterCurrency)) {
                            availableCurrencies.add(baseCurrency);
                        }
                    }
                }
     ...
     * WorldcoinChartsExchange.class.getName();
     * 
    ...
    private Exchange createExchange(String exchangeShortname) {
        ...
        try {
            ...
            // but error should not spread out from here to rest of WorldcoinWallet.
            log.error(e.getClass().getName() + " " + e.getMessage());
