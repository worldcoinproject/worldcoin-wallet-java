...
public class Localiser {
    ...
    public static final String WORLDCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY = "/i18n";
    public static final String WORLDCOIN_WALLET_RESOURCE_BUNDLE_NAME = "viewer";
    ...
    public static final String LANGUAGE_PROPERTIES_FILENAME = WORLDCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/language.properties";
    ...
    public static final int NUMBER_OF_FRACTION_DIGITS_FOR_WORLDCOIN = 8;
    ...
    public Localiser(Locale locale) {
        ...
        numberFormat.setMaximumFractionDigits(NUMBER_OF_FRACTION_DIGITS_FOR_WORLDCOIN);
        numberFormatNotLocalised = NumberFormat.getInstance(Locale.ENGLISH);
        numberFormatNotLocalised.setMaximumFractionDigits(NUMBER_OF_FRACTION_DIGITS_FOR_WORLDCOIN);
    ...
    public void setLocale(Locale locale) {
        String propertyFilename = WORLDCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/" + locale.getLanguage() + "/"
                + WORLDCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;
        String propertyFilenameBase = WORLDCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/" + FALLBACK_LANGUAGE_CODE + "/"
                + WORLDCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;

        if ("he".equals(locale.getLanguage()) || "iw".equals(locale.getLanguage())) {
            // Hebrew can be he or iw
            this.locale = new Locale("iw");
            propertyFilename = WORLDCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/he/" + WORLDCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;
        } if ("id".equals(locale.getLanguage()) || "in".equals(locale.getLanguage())) {
            // Indonesian can be id or in
            this.locale = new Locale("in");
            propertyFilename = WORLDCOIN_WALLET_RESOURCE_BUNDLE_DIRECTORY + "/id/" + WORLDCOIN_WALLET_RESOURCE_BUNDLE_NAME + PROPERTY_NAME_SUFFIX;
        } else {
            this.locale = locale;
        }
     ...
     **/
    public String worldcoinValueToString(BigInteger value, boolean addUnit, boolean blankZero) {
        ...
        toReturn = toReturn + numberFormat.format(valueInWDC.doubleValue());

        if (addUnit) {
            toReturn = toReturn + " " + getString("sendWorldcoinPanel.amountUnitLabel");
        }
        return toReturn;     
    }
    /**
     ...
     **/
    public String worldcoinValueToStringNotLocalised(BigInteger value, boolean addUnit, boolean blankZero) {
        if (blankZero && value.compareTo(BigInteger.ZERO) == 0) {
            return "";
        }
        ...
        BigDecimal valueInWDC = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        toReturn = toReturn + numberFormatNotLocalised.format(valueInWDC.doubleValue());
