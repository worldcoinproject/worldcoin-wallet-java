...
public class QRCodeGenerator {
    ...
    public QRCodeGenerator(WorldcoinController worldcoinController) {
     ...
     * 
     * @param address
     *            Worldcoin address to show
     * @param amount
     *            amount of WDC to show - text
     * @param label
     *            label for swatch
     * @return
     */
    public BufferedImage generateQRcode(String address, String amount, String label, int scaleFactor) {
        ...
        try {
            ...
            if (address != null && !"".equals(address) && this.worldcoinController.getWorldcoinWalletService() != null
                    && this.worldcoinController.getModel().getNetworkParameters() != null) {
                decodeAddress = new Address(this.worldcoinController.getModel().getNetworkParameters(), address);
            }
            if (decodeAddress != null && !"".equals(decodeAddress)) {
                if (amount != null && !"".equals(amount)) {
                    ...
                    if (converterResult.isWdcMoneyValid()) {
                        worldcoinURI = WorldcoinURI.convertToWorldcoinURI(decodeAddress, converterResult.getWdcMoney().getAmount().toBigInteger(), label, null);
                    } else {
                       // No parsable amount - show nothing.  
                    }                } else {
                    worldcoinURI = WorldcoinURI.convertToWorldcoinURI(decodeAddress, null, label, null);
                }
            }
            this.worldcoinController.getModel().setActiveWalletPreference(WorldcoinModel.SEND_PERFORM_PASTE_NOW, "false");
        } catch (IllegalArgumentException e) {
            //log.warn("The address '" + address + "' could not be converted to a worldcoin address. (IAE)");
            return null;
        } catch (AddressFormatException e) {
            //log.warn("The address '" + address + "' could not be converted to a worldcoin address. (AFE)");
            return null;
        }
